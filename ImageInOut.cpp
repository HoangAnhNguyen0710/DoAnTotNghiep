...
void save_image(const char* output_filename,
    ...
    int height,
    int width) {
    cv::Mat output_image(height, width, CV_32FC3, buffer); // Sử dụng CV_32FC1 cho ảnh xám 1 kênh

    // Make negative values zero.
    cv::threshold(output_image, output_image, /*threshold=*/0, /*maxval=*/0, cv::THRESH_TOZERO);
    cv::normalize(output_image, output_image, 0.0, 255.0, cv::NORM_MINMAX);
    output_image.convertTo(output_image, CV_32FC3); // Chuyển đổi sang kiểu dữ liệu 8-bit unsigned integer
